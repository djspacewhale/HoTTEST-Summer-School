...
Bowtie-rec : {l : Level} {X : Type l}
             ...
             (q : x ≡ x [ X ])
           → (Bowtie) → X
Bowtie-rec {_} {X} x p q = Bowtie-elim (λ _ → X) x (PathOver-constant _ p) (PathOver-constant _ q)
...
Bowtie-rec-base {l} {X} x p q = refl _
```
...
concat-equiv : ∀ {A : Type} (a : A) {a' a'' : A}
                     → (p : a' ≡ a'')
                     → (a ≡ a') ≃ (a ≡ a'')
concat-equiv a (refl _) = Equivalence id (Inverse id (λ x → refl x) id λ x → refl x)
...
transport-∙ (refl _) (refl _) x = refl x
```
